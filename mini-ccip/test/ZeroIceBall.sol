...
contract ZeroIceBall is Test {
    ...
    // ================================================================
    function testFuzz_ZeroIceBall(uint8 rawA, uint8 rawB, uint8 rawC) public {
    ...
    function test_ZeroIceBall_QuickBatch() public {
    ...
    function test_ZeroIceBall_QuickBatch_ExcludeKnown() public {
    ...
    function test_ZeroIceBall_TripleScan_Part1() public { console2.log("=== TRIPLE SCAN Part 1/9 SAMPLED (i=0..12) ==="); _tripleScanRangeSampled(0, 13); }
    function test_ZeroIceBall_TripleScan_Part2() public { console2.log("=== TRIPLE SCAN Part 2/9 SAMPLED (i=13..25) ==="); _tripleScanRangeSampled(13, 26); }
    function test_ZeroIceBall_TripleScan_Part3() public { console2.log("=== TRIPLE SCAN Part 3/9 SAMPLED (i=26..38) ==="); _tripleScanRangeSampled(26, 39); }
    function test_ZeroIceBall_TripleScan_Part4() public { console2.log("=== TRIPLE SCAN Part 4/9 SAMPLED (i=39..51) ==="); _tripleScanRangeSampled(39, 52); }
    function test_ZeroIceBall_TripleScan_Part5() public { console2.log("=== TRIPLE SCAN Part 5/9 SAMPLED (i=52..64) ==="); _tripleScanRangeSampled(52, 65); }
    function test_ZeroIceBall_TripleScan_Part6() public { console2.log("=== TRIPLE SCAN Part 6/9 SAMPLED (i=65..77) ==="); _tripleScanRangeSampled(65, 78); }
    function test_ZeroIceBall_TripleScan_Part7() public { console2.log("=== TRIPLE SCAN Part 7/9 SAMPLED (i=78..90) ==="); _tripleScanRangeSampled(78, 91); }
    function test_ZeroIceBall_TripleScan_Part8() public { console2.log("=== TRIPLE SCAN Part 8/9 SAMPLED (i=91..102) ==="); _tripleScanRangeSampled(91, 103); }
    function test_ZeroIceBall_TripleScan_Part9() public { console2.log("=== TRIPLE SCAN Part 9/9 SAMPLED (i=103..113, new injections) ==="); _tripleScanRangeSampled(103, 114); }
    ...
    // ================================================================
    function test_ZeroIceBall_Stack4_KnownFindings() public {
    ...
    function test_ZeroIceBall_Stack5_KnownFindings() public {
    ...
    function test_ZeroIceBall_Stack6_KnownFindingsPlusControl() public {
    ...
    //
    // Context: test_ZeroIceBall_IterativeDeepeningUntilBug still reports
    ...
    // ================================================================
    function test_ZeroIceBall_Stack6_KnownFindingsPlusDROP01() public {
    ...
    // ================================================================
    function test_ZeroIceBall_IterativeDeepeningUntilBug() public {
    ...
    // ================================================================
    // MAXIMAL ITERATIVE DEEPENING - fresh EVM state at EVERY depth.
    //
    // The original test_ZeroIceBall_IterativeDeepeningUntilBug() ran all
    // 114 depths sequentially in ONE persistent EVM state - which we
    // proved (via the DROP01 isolation test) causes real contamination:
    // an earlier depth's side effects can suppress a finding that would
    // fire cleanly if tested in isolation. It also stopped at the FIRST
    // genuinely-new finding, never exploring past it.
    //
    // This version fixes both: _freshDeploy() completely redeploys every
    // mock contract before EACH depth (identical to what setUp() does),
    // so depth D can never contaminate depth D+1. It also does NOT stop
    // early - it runs the full 1..114 range regardless, logging every
    // genuinely-new finding at every depth it appears, then explicitly
    // runs the full N=114 simultaneous stack as the terminal case.
    //
    // This is the literal, honest ceiling of "injection inside injection
    // inside injection..." - there is no such thing as infinite depth
    // when there are only 114 known injections; this IS the maximum,
    // run as rigorously as the DROP01 isolation test proved was necessary.
    // ================================================================
    ...
    function _freshDeploy() internal {
        ...
        uln = new UlnMock(address(dstEndpoint));
        dstEndpoint.setUln(address(uln));
        ulnOld = new UlnMock(address(dstEndpoint));
        srcApp = new OAppToken(address(srcEndpoint));
        dstApp = new OAppToken(address(dstEndpoint));
        dvnA = new DVN(address(uln), false);
        dvnB = new DVN(address(uln), false);
        dvnMalicious = new DVN(address(uln), true);
        dvnA.setMinFee(100); dvnB.setMinFee(100);
        vm.deal(address(dstEndpoint), 10 ether);
        ...
        dstApp2 = new OAppToken(address(dstEndpoint));
        {
            address[] memory weakReq = new address[](1); weakReq[0] = address(dvnB);
            address[] memory noOpt = new address[](0);
            ulnOld.setPathway(address(dstApp2), SRC_EID, bytes32(uint256(uint160(address(srcApp)))), weakReq, noOpt, 0, 5);
    ...
    function test_ZeroIceBall_MaximalIterativeDeepening_FreshStatePerDepth() public {
        console2.log("=== MAXIMAL ITERATIVE DEEPENING: fresh state per depth, 1 through", N, "===");
        console2.log("(no early stopping - runs the FULL range, logs every genuinely-new finding at every depth)");

        uint256 totalGenuinelyNewAcrossAllDepths = 0;

        for (uint8 depth = 1; depth <= N; depth++) {
            _freshDeploy();
            ...
            bool anyNewThisDepth = (critCount > critBefore) || (highCount > highBefore) || (medCount > medBefore);

            if (found && anyNewThisDepth) {
                console2.log("Depth", depth, "- NEW finding(s) surfaced (fresh state, not contaminated)");
                totalGenuinelyNewAcrossAllDepths++;
            } else if (found) {
                console2.log("Depth", depth, "- re-hit only already-seen finding(s)");
            }

            if (depth == N) {
                console2.log("=== TERMINAL CASE: all", N, "injections active simultaneously, fresh state ===");
                console2.log("This is the maximum possible depth - there is no 115th injection to add.");
            }
        }

        console2.log("=== SWEEP COMPLETE ===");
        console2.log("Depths that produced at least one NEW finding (fresh-state, uncontaminated):", totalGenuinelyNewAcrossAllDepths);
        console2.log("Final cumulative tally - Critical:", critCount, "High:", highCount);
        console2.log("Final cumulative tally - Medium:", medCount, "Low:", lowCount);
        console2.log("Total distinct combinations actually run:", totalRun);
    }


    function test_ZeroIceBall_MaximalIterativeDeepening_V2_ConflictAware() public {
        // V1 was BROKEN in a way that wasn't caught before presenting the
        // result: _runArr silently returns false (no test run at all) if
        // the ids array contains BOTH members of a conflicting pair
        // (E60-E63 decimal group, or F75/F76 peer group). Sequential
        // [0..depth-1] IDs mean once depth reaches 28, BOTH F75(27) and
        // F76(26) are always in the set together - so every depth from 28
        // through 114 (87 of them) was silently skipped, contributing
        // ZERO real tests. totalRun=27 in V1 proves this exactly: only
        // depths 1-27 ever actually ran.
        //
        // Fixed here: build each depth's ID set by walking through 0..113
        // in order and greedily including an ID UNLESS it would conflict
        // with something already in the set - so the set keeps growing to
        // genuinely deep, conflict-free combinations instead of dying.
        console2.log("=== MAXIMAL ITERATIVE DEEPENING V2: conflict-aware, fresh state per depth ===");

        uint8[] memory activeSet = new uint8[](N);
        uint8 activeCount = 0;
        uint256 totalGenuinelyNewAcrossAllDepths = 0;
        uint256 realRunsExecuted = 0;

        for (uint8 candidate = 0; candidate < N; candidate++) {
            bool conflictsWithSet = false;
            for (uint8 i = 0; i < activeCount; i++) {
                if (_conflicts(activeSet[i], candidate)) { conflictsWithSet = true; break; }
            }
            if (conflictsWithSet) {
                console2.log("Injection ID", candidate, "conflicts with an already-active ID - skipped for this sweep, not tested at this depth");
                continue;
            }

            activeSet[activeCount] = candidate;
            activeCount++;

            _freshDeploy();

            uint8[] memory ids = new uint8[](activeCount);
            for (uint8 i = 0; i < activeCount; i++) { ids[i] = activeSet[i]; }
            ...
            bool anyNewThisRun = (critCount > critBefore) || (highCount > highBefore) || (medCount > medBefore);

            if (found && anyNewThisRun) {
                console2.log("Active count", activeCount, "- NEW finding(s) surfaced (fresh state, conflict-free)");
                totalGenuinelyNewAcrossAllDepths++;
            } else if (found) {
                console2.log("Active count", activeCount, "- re-hit only already-seen finding(s)");
            }
        }

        console2.log("=== TERMINAL: reached maximum conflict-free simultaneous count:", activeCount);
        console2.log("...out of total injections available:", N);
        console2.log("=== SWEEP COMPLETE (V2, conflict-aware) ===");
        console2.log("Real runs actually executed (should be much closer to N than V1's 27):", realRunsExecuted);
        console2.log("Runs that produced at least one NEW finding:", totalGenuinelyNewAcrossAllDepths);
        console2.log("Final cumulative tally - Critical:", critCount, "High:", highCount);
        console2.log("Final cumulative tally - Medium:", medCount, "Low:", lowCount);

        assertTrue(realRunsExecuted > 27, "V2 fix did not actually improve real-run coverage versus V1's broken 27 - investigate _conflicts() logic further");
    }
